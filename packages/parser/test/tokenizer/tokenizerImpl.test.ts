...
import { SPEC, TokenTypes } from "../../src/constants/jokerlangSpec";
import InvalidStateException from "../../src/exceptions/invalidStateException";

const tokenizer = new TokenizerImpl(SPEC);

test("test Tokenizer.getNextToken with HI_joker_TYPE should success", () => {
  tokenizer.initTokenizer(`hi joker`);

  expect(tokenizer.getNextToken()).toStrictEqual({
    type: TokenTypes.HI_joker_TYPE,
    value: "hi joker",
  });
});

test("test Tokenizer.getNextToken with BYE_joker_TYPE should success", () => {
  tokenizer.initTokenizer(`bye joker`);

  expect(tokenizer.getNextToken()).toStrictEqual({
    type: TokenTypes.BYE_joker_TYPE,
    value: "bye joker",
  });
});
